...
class App:
    ...
    def main(self):
        ...
        model = YOLO("../model_weights/detect/l_best_body.pt")
        ...
        while True:
            ...
            if counter >= startFrame:
                results = model.predict(source=frame.copy(), save_crop=False, save=False, save_txt=False, stream=True, conf=0.35, verbose=True, max_det = 1)
                for result in results:
                    if len(result.boxes) != 0:
                        img = frame.copy()
                        img[:] = 0
                        print(result.boxes.xyxy)
                        coords = result.boxes.xyxy[0].cpu().detach().numpy().astype(int)
                        maskr = cv2.rectangle(img, (coords[0], coords[1]), (coords[2], coords[3]), color=(255,255,255), thickness=-1)
                        mask = cv2.cvtColor(maskr, cv2.COLOR_BGR2GRAY)
                        outputh = cv2.bitwise_and(result.orig_img, result.orig_img, mask=mask)

                        book = result.orig_img[coords[1]:coords[3]+1, coords[0]:coords[2]+1]
                        book_mask = mask[coords[2]:coords[3]+1, coords[0]:coords[1]+1]
                    else:
                        book = old_book
                        book_mask = old_mask
                        coords = old_coords
